...
- **Per-class F1:** B-Skill / I-Skill / B-Knowledge / I-Knowledge / O
...
- **Per-class F1 (test):** O = 0.94 · B-Skill = **0.51** (+0.02) · I-Skill = 0.55 (—) · B-Knowledge = 0.63 (−0.01) · I-Knowledge = 0.51 (—)
- **Notes:** Mixed but slightly positive. Val and recall improved (B-Skill jumped +0.02). The task-adapted embedding directions help the Skill class specifically; Knowledge slightly regressed — possibly because the teacher's Knowledge fine-tuning was less consistent than its Skill-class adjustments. Explained variance after PCA was identical to vanilla (40.32%) because fine-tuning doesn't change BERT's eigenvalue spectrum much; the eigenvectors are what move.
...
- **Per-class F1 (test):** O = 0.94 · B-Skill = 0.49 (—) · I-Skill = **0.55** (+0.01) · B-Knowledge = **0.64** (+0.02) · **I-Knowledge = 0.51** (+0.05, biggest single-class jump in the entire optimization run)
...
- **Per-class F1 (test):** O = 0.94 · B-Skill = 0.49 · I-Skill = 0.54 · B-Knowledge = **0.62** (+0.01) · I-Knowledge = 0.46 (−0.01)
...
- **Per-class F1 (test):** O = 0.94 · B-Skill = **0.49** (+0.02) · I-Skill = 0.52 (—) · B-Knowledge = **0.60** (+0.01) · I-Knowledge = **0.48** (+0.01)
- **Teacher F1 (test, BERT v2):** macro 0.75 · B-Skill 0.69 · I-Skill 0.69 · B-Knowledge 0.77 · I-Knowledge 0.64
...
- **Per-class F1 (test):** O = 0.94 · B-Skill = **0.47** (+0.01) · I-Skill = **0.52** (+0.02) · B-Knowledge = **0.59** (+0.02) · I-Knowledge = **0.47** (+0.01)
- **Inference:** Total test inference 9.97s on CPU (slightly faster than Fix #1's 11.13s — likely warmup noise; arch is unchanged)
- **Notes:** **Modest but real lift across every skill class.** Recall moved more than precision (+0.01 on macro recall, +0.01 on macro P) — consistent with "more data → more skill tokens seen → finds more spans." Skill got the biggest absolute gain (I-Skill +0.02), supporting the diagnosis that Skill was data-limited. Smaller-than-hoped overall lift likely because pseudo set is only 586 examples (~12% data boost); a 5-10× larger unlabeled corpus would likely push further. Convergence at epoch 32 vs. 45 hints the extra signal was useful (model learned faster) rather than noisy.

## 2026-05-10 — Fix #2: Biaffine span head (no KD)
- **Change:** Replaced BIO+CRF with a biaffine span scorer over (start, end) token pairs → {None, Skill, Knowledge}. New module `BiaffineSpanHead`, model class `SpanSkillExtractor`, training script `train_biaffine.py`. Greedy non-overlapping decoding. KD intentionally not wired in v1.
- **Config:** 6 layers / d_model 128 / d_ff 768 / d_span_proj 64 / max_span_len 20 / weighted CE (None=0.1, Skill=1.0, Knowledge=1.0) / KD off
...
- **Per-class F1:** TBD (only macro reported; per-Skill/Knowledge breakdown not yet logged)
- **Inference:** TBD
...
- **Per-class span F1 (test):** Skill = **0.1906** (P 0.2237 / R 0.1661) · Knowledge = **0.4169** (P 0.4044 / R 0.4302)
- **Notes:** Hypothesis on overall F1 confirmed: P went 0.25 → 0.33 (+0.08), R went 0.36 → 0.30 (−0.06), F1 went 0.30 → 0.32 (+0.02). **But the per-class story is the opposite of what I predicted.** I hypothesized biaffine would help Skill specifically (where BIO struggled at 0.24 due to B/I asymmetry). Reality: biaffine is **worse on Skill (0.19 vs 0.24)**, not better. The gap is also slightly larger on Skill (−0.052) than on Knowledge (−0.034). **Revised diagnosis:** the BIO B/I asymmetry was a symptom, not the cause. The real bottleneck on Skill is training signal (KD doing heavy lifting in BIO+CRF), not the labeling scheme. Biaffine without KD just lacks that signal uniformly — slightly more on Skill because Skill is the harder, more ambiguous class.
...
- **Per-class span F1:** Skill = **0.2424** (P 0.2656 / R 0.2229) · Knowledge = **0.4505** (P 0.4749 / R 0.4284)
- **Notes:** Token-F1 0.59 → Span-F1 0.35 — confirms span-F1 is a much stricter metric (~24-point compression). Skill is the weak class (0.24); Knowledge is decent (0.45). Span Precision > Span Recall (under-predicts), matching the token-level pattern. **Direct comparison:** biaffine without any KD gets 0.30 — only 5 points behind a fully-tuned BIO+KD baseline. Biaffine has real headroom once KD is added.
...
- **Notes:** +9 F1 over pre-KD baseline. Residual issues: subword artifacts ("Dock", "Ku") in inference output, and category confusion ("Python" sometimes tagged Knowledge instead of Skill).
